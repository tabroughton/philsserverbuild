...
** DNS for internet access (if needed)
- duckdns.org
- you need to copy =./duckdns.env.template= to =/etc/duckdns.env=
- set the token in =/etc/duckdns.env=
...
5. Explicitly expose public services *only when needed*
   - note these ports and forwarding would need setting on router too
6. Set duckdns.org (philshomeserver.duckdns.org)
   - copy duckdns.env.template to /etc/duckdns.env and edit the token
